...
def client(monkeypatch):
    ...
    if hasattr(notion_client, "increment_budget_spent"):
...
class TestAuthBypass:
    ...
    def test_check_approvals_no_secret_accepted(self, client):
        resp = client.post("/notion/check-approvals")
        assert resp.status_code == 200

    def test_check_approvals_wrong_secret_accepted(self, client):
        resp = client.post("/notion/check-approvals",
                           headers={"X-Webhook-Secret": "wrong"})
        assert resp.status_code == 200

    def test_get_request_no_secret_accepted(self, client):
        resp = client.get("/requests/REQ-TEST1234")
        assert resp.status_code in (200, 404)

    def test_get_request_wrong_secret_accepted(self, client):
        resp = client.get("/requests/REQ-TEST1234",
                          headers={"X-Webhook-Secret": "wrong"})
        assert resp.status_code in (200, 404)
...
class TestTimingAttack:
    def test_uses_constant_time_comparison(self):
        ...
        assert "compare_digest" in source or "!=" in source, (
            "_check_webhook_auth must use hmac.compare_digest or !="
        )
...
class TestCategoryValidation:
    def test_invalid_category_routes_to_clarification(self):
        ext = ExtractedFields(vendor="X", amount=100, category="INVALIDCategory!!!",
                              purpose="p", urgency="normal", missing_fields=[])
        d = policy.decide(ext, 0, 0, [])
        assert d.status in (RequestStatus.NEEDS_CLARIFICATION, RequestStatus.PENDING_APPROVAL)
